trigger SDO_CPQ_SBQuote on SBQQ__Quote__c (after insert, after update) {
    if(!System.isBatch()) return;
    if(trigger.isAfter && trigger.isInsert){

        SBQQ__Quote__c []newQuote = Trigger.new;
        SDO_FSL_blogic_Quote_CreateLineItems.InsertAfter(newQuote);
        }   
    ...
    if(trigger.isAfter && trigger.isUpdate){
        ...
        if(newQuoteList.size() > 0){
          SDO_FSL_blogic_Quote_CreateLineItems.UpdateAfter(newQuoteList);
        }
        }   
