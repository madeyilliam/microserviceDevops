trigger SDO_Tool_Chatter_feedItemSearchWatchWordTrigger on FeedItem (after insert) 
{
    ...
    { 
        for(String watchWord : watchwords)
            if(f.Body!= null && f.Body.containsIgnoreCase(watchWord)) //check if the feeditem body is null
            {
